...
export async function listTokensForUser(email, password) {
  return sendPostRequestToCredentials(JSON.stringify({ email, password }));
}
...
export async function getAccessToken(id, secret) {
  ...
  const dpopHeader = await createDpopHeader(
    TOKEN_ENDPOINT,
    "POST",
    dpopKeyPair
  );
  ...
  if (!response.ok) {
    throw new Error(
      `Getting new access token failed: ${await response.text()}`
    );
  }
